...
public class user_request extends HttpServlet
{
    ...
    String x,z,m,a1,a2,a3;
    a1=request.getParameter("bg");
    a2=request.getParameter("res");
    a3=hs.getAttribute("remail").toString();
     boolean y=false;
   
    
    x="insert into request(email,bgroup,reason) values('"+a3+"','"+a1+"','"+a2+"')";
    z="update request set bgroup='"+a1+"',reason='"+a2+"'where email='"+a3+"'";
    m="select email from request where email='"+a3+"'";
  try{
